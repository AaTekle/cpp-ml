...
* `/O2` — Enables speed-focused optimization in Microsoft Visual C++.
* `/W4` — Enables a high level of compiler warnings in Microsoft Visual C++.
...
* `-O1` — Enables basic compiler optimization.
* `-O2` — Enables strong optimization with a balance between speed and executable size.
* `-O3` — Enables aggressive speed-focused optimization in GCC and Clang.
* `-Os` — Optimizes the program to reduce executable size.
* `-Og` — Enables optimization while preserving a more useful debugging experience.
* `-Wall` — Enables a broad set of common compiler warnings.
